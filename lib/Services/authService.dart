...
class AuthService{
  ...
  Future<void> updateUserInfos(String name, String surname, String email)async{
    try {
      await users.doc(uid).update({"name":name,"surname":surname, "email":email});
    }catch (e) {
  ...
  Future<void> updateSubscription(int type)async{
    if(type==1){
      ...
      await users.doc(uid).update({"subscriptionType":"Günlük","subscriptionStarting":Timestamp.now(),"subscriptionEnding":Timestamp.fromDate(tomorrow)});
    }
    else if(type ==2){
      DateTime now = DateTime.now();
      DateTime nextWeek = now.add(Duration(days: 7));
      await users.doc(uid).update({"subscriptionType":"Haftalık","subscriptionStarting":Timestamp.now(),"subscriptionEnding":Timestamp.fromDate(nextWeek)});
    }
    else if(type ==3){
      DateTime now = DateTime.now();
      DateTime nextMonth = now.add(Duration(days: 30));
      await users.doc(uid).update({"subscriptionType":"Aylık","subscriptionStarting":Timestamp.now(),"subscriptionEnding":Timestamp.fromDate(nextMonth)});
    }
  }
  ...
  Future<void> register(String phoneNumber, String uid) async {
    Map<String, dynamic> usermap = {
      ...
      "ppLink": "https://firebasestorage.googleapis.com/v0/b/juniorapp-99648.appspot.com/o/defaultPP%2F360_F_128563455_bGrVZnfDCL0PxH1sU33NpOhGcCc1M7qo.jpg?alt=media&token=14ceeeae-fce3-4288-8ec9-ccd83c65b04c",
  ...
  Future<void> sendOpinion(String opinion) async {
    ...
    await FirebaseFirestore.instance.collection("opinions").add({
      ...
      await FirebaseFirestore.instance.collection("opinions").doc(value.id).update({"UID": value.id});
    });
  }
